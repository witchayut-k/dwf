...
/*!******************************************************!*\
  !*** ./resources/assets/backend/js/pages/content.js ***!
  \******************************************************/
var Content = function () {
    ...
    datatable = $('#table-content').DataTable({
      ...
      columns: [{
        ...
        render: function render(data, type, row) {
          var content = row.pinned ? data + ' <i class="fa fa-thumb-tack text-danger tooltips" title="เนื้อหาปักหมุด"></i>' : data;
          return content;
        }
      }, {
        data: 'featured_image',
        sClass: 'text-center',
        render: function render(data, type, row) {
          return "<img src=\"".concat(data, "\" style=\"width: 118px; height: auto;\" />");
        }
      }, {
        data: 'type_name',
        sClass: 'text-center'
  ...
  var handleContentTypeChange = function handleContentTypeChange() {
    $('[name="content_type_id"]').on('change', function () {
  ...
  var initFiles = function initFiles() {
    if (!contentId) return;
    $.ajax({
      url: "".concat(appUrl, "/api/backend/contents/").concat(contentId, "/files"),
      type: 'GET',
  ...
  var initFilesUpload = function initFilesUpload() {
    initFiles();
    var dzFiles = $(".btn-add-files");
    dzFiles.dropzone({
      url: "".concat(moduleUrl, "/").concat(contentId, "/uploads/files"),
      ...
      init: function init() {
        ...
        this.on("thumbnail", function (file) {// setTimeout(() => {
  ...
  var handleDeleteFiles = function handleDeleteFiles() {
    $('body').on('click', '.btn-delete-item', function () {
      contentApp.files.processing = true;
      $.ajax({
        url: "".concat(moduleUrl, "/").concat(contentId, "/files"),
        type: 'delete',
        data: {
  ...
  return {
    init: function init() {
      ...
      if ($('#form-content').length) {
        customSubmit = true; // handleFileUpload();
